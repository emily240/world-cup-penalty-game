...
export default function Home({mobileMode=false}:{mobileMode?:boolean}){
  ...
  useEffect(()=>{
    ...
    G.reset=(full=false)=>{G.reactionTimers.forEach(clearTimeout);G.reactionTimers=[];originalReset(full);G.savedPose=false;G.deflect=null;G.keeper.adjusting=false;G.ball.y=525;G.phase="placeBall";G.player={x:835,y:630};G.targetRel={x:-145+Math.random()*290,y:-48+Math.random()*96,vx:(Math.random()<.5?-1:1)*(26+Math.random()*12),vy:(Math.random()<.5?-1:1)*(18+Math.random()*9)};if(full){G.goalOffset=0;G.goalOffsetY=0;G.goalVelocity=135;G.goalVelocityY=-90}};
  ...
  return <main className={mobileMode?"game-shell mobile-game":"game-shell"}>
    ...
    {screen==="instructions"&&<div className="overlay"><section className="card rules"><p className="eyebrow">MATCH BRIEFING</p><h2>HOW TO PLAY</h2><ol><li><b>PLACE & PULL BACK</b><span>Distance sets shot speed; sideways movement sets aim. The aim marker is the exact landing point.</span></li><li><b>SCORE 5 IN 60 SECONDS</b><span>Slow shots give the goalkeeper more time to adjust. After 2 goals, the goal moves.</span></li><li><b>FINAL TARGET</b><span>After 4 goals, hit the bullseye for the winning goal. The target keeps moving while the ball is in flight.</span></li></ol><div className="rulebox">Reach 5 goals before time runs out. On the final shot, only a direct bullseye hit counts.</div><Button onClick={start}>START →</Button><button className="textbtn" onClick={menu}>← BACK TO MAIN MENU</button></section></div>}
