...
export default function Home({mobileMode=false}:{mobileMode?:boolean}){
  ...
  useEffect(()=>{
    ...
    G.phase="placeBall";
    G.goalOffset=0;G.goalOffsetY=0;G.goalVelocity=135;G.goalVelocityY=-90;G.goalLast=performance.now()/1000;G.target={x:640,y:210};G.targetRel={x:0,y:0,vx:14,vy:10};
    G.ball.y=525;G.player={x:835,y:630};G.reactionTimers=[];
    G.launch=()=>{originalLaunch();if(G.to){if(statsRef.current.goals>=4){G.to.x=G.aim.x;G.to.y=G.aim.y}else G.to.x+=(Math.random()-.5)*(24+G.power*48);const reaction=window.setTimeout(()=>{if(G.state==="flight"&&statsRef.current.goals<4&&Math.random()<.9)G.keeper.v=(G.to.x-G.goalOffset>G.keeper.x?1:-1)*(430+Math.random()*260)},90+Math.random()*120);G.reactionTimers.push(reaction)}};
    G.resolve=()=>{
      ...
      const react=Math.max(.12,G.shotDur-.2),reachable=Math.abs(x-G.keeper.x)<(175+react*120),saved=!targetMode&&reachable&&Math.random()<(.76-G.power*.14),hitTarget=!targetMode||Math.hypot(G.to.x-G.target.x,G.to.y-G.target.y)<48;
    ...
    G.pointer=(type:string,e:PointerEvent)=>{
      ...
      if(type==="move"&&G.drag&&G.state==="charge"){
        G.player.x=Math.max(430,Math.min(850,x));G.player.y=Math.max(G.ball.y+76,Math.min(704,y));
        const back=Math.max(0,G.player.y-G.ball.y-50),side=G.player.x-G.ball.x;
        G.power=Math.max(.12,Math.min(1,Math.hypot(back,side*.45)/145));
      ...
      if(type==="up"&&G.drag){
        ...
        else if(G.state==="charge"&&G.power>.16){G.keeper.willDive=statsRef.current.goals<4&&Math.random()<.78;audio.current.shout();G.shoot();}
  ...
  return <main className={mobileMode?"game-shell mobile-game":"game-shell"}>
    ...
    {screen==="playing"&&<><div className="hud"><div><small>TIME</small><b className={stats.time<=10?"danger":""}>{Math.ceil(stats.time)}</b></div><div><small>GOALS</small><b>{stats.goals}<em> / 5</em></b></div><div className="target"><small>{stats.goals>=4?"FINAL TARGET":"TARGET"}</small><b>{stats.goals>=4?"HIT BULLSEYE":"5 GOALS"}</b></div></div><button className="pause" aria-label="Pause game" onClick={()=>setScreen("paused")}>Ⅱ</button>{result&&<div className={`result ${result==="GOAL!"?"goal":""}`}>{result}</div>}</>}
    {screen==="menu"&&<div className="overlay"><section className="card hero"><div className="cup">★<span>WORLD<br/>CHAMPIONS</span></div><p className="eyebrow">THE FINAL</p><h1>TAKE<br/><span>THE SHOT.</span></h1><p className="lead match-rule">5 GOALS · 60 SECONDS · WIN THE CUP</p><div className="actions"><Button onClick={start}>START <span>→</span></Button><Button secondary onClick={()=>setScreen("instructions")}>HOW TO PLAY</Button></div><div className="toggles"><button onClick={()=>setMusic(!music)}>♫ MUSIC <b>{music?"ON":"OFF"}</b></button><button onClick={()=>setSound(!sound)}>◖ SOUND <b>{sound?"ON":"OFF"}</b></button></div></section></div>}
    {screen==="instructions"&&<div className="overlay"><section className="card rules"><p className="eyebrow">MATCH BRIEFING</p><h2>HOW TO PLAY</h2><ol><li><b>PLACE & PULL BACK</b><span>Place the ball, then pull Yamal backward. Distance sets power; sideways movement sets aim.</span></li><li><b>SCORE 5 IN 60 SECONDS</b><span>After 2 goals, the goal moves left and right.</span></li><li><b>FINAL TARGET</b><span>After 4 goals, hit the bullseye for the winning goal. The aim marker exactly matches the shot landing point, and the goal moves at half speed.</span></li></ol><div className="rulebox">Reach 5 goals before time runs out. On the final shot, only a direct bullseye hit counts.</div><Button onClick={start}>START →</Button><button className="textbtn" onClick={menu}>← BACK TO MAIN MENU</button></section></div>}
